...
extern timeInterval_t vTimeTransfer;
extern uint32_t time_tick;
extern TIM_HandleTypeDef htim2;

void Lora_Init(LoRa *pL, SPI_HandleTypeDef *phspi, uint16_t frequency, uint8_t LORA){
	pL->hSPIx                 = phspi;
	
	if(LORA == LORA_TX){
		pL->CS_port               = NSS_TX_GPIO_Port;
		pL->CS_pin                = NSS_TX_Pin;
		pL->reset_port            = RESET_TX_GPIO_Port;
		pL->reset_pin             = RESET_TX_Pin;
	}
	else if(LORA == LORA_RX){
		pL->CS_port               = NSS_RX_GPIO_Port;
		pL->CS_pin                = NSS_RX_Pin;
		pL->reset_port            = RESET_RX_GPIO_Port;
		pL->reset_pin             = RESET_RX_Pin;
		pL->DIO0_port						 = DIO0_IRQ_LORA_GPIO_Port;
		pL->DIO0_pin							 = DIO0_IRQ_LORA_Pin;

	}
	
	pL->frequency             = frequency;							  // default = 433 MHz
	pL->spredingFactor        = SF_7;							// default = SF_7
	pL->bandWidth			       = BW_500KHz;       // default = BW_125KHz
	pL->crcRate				       = CR_4_5;						// default = CR_4_5
	pL->power					       = POWER_20db;				// default = 20db
	pL->overCurrentProtection = 120; 							// default = 100 mA
	pL->preamble				       = 10;		  					// default = 8;
}
...
void LoRa_gotoMode(LoRa* _LoRa, int mode){
	...
	}else if (mode == FSTX_MODE){
		data = (read & 0xF8) | 0x02;
		_LoRa->current_mode = FSTX_MODE;
	}else if (mode == FSRX_MODE){
		data = (read & 0xF8) | 0x04;
		_LoRa->current_mode = FSRX_MODE;
	}
...
uint8_t FSK_transmit(LoRa* _LoRa, uint8_t* data, uint8_t length, uint16_t timeout){
	uint8_t read;
...
//	LoRa_write(_LoRa, RegPayloadLength, length);
	...
	//LoRa_gotoMode(_LoRa, FSTX_MODE);
	LoRa_gotoMode(_LoRa, TRANSMIT_MODE);
	
	while(1){
		read = LoRa_read(_LoRa, FSK_RegIrqFlags2);
		if((read & 0x08)!=0){
			LoRa_write(_LoRa, FSK_RegIrqFlags2, (read & 0xF7));
			LoRa_gotoMode(_LoRa, mode);
			return 1;
		}
		else{
...
void FSK_startReceiving(LoRa* _LoRa){
	LoRa_gotoMode(_LoRa, FSRX_MODE);
}
void FSK_startTrasmiting(LoRa* _LoRa){
	LoRa_gotoMode(_LoRa, FSTX_MODE);
}
...
/* ----------------------------------------------------------------------------- *\
		...
		returns     : Nothing
\* ----------------------------------------------------------------------------- */
uint16_t LoRa_Config(LoRa* _LoRa){
	uint8_t    data;
	uint8_t    read;
...
/*-------------------------------------------------------------------*/
uint16_t FSK_Config(LoRa* _LoRa){
	uint8_t    data;
	uint8_t    read;

	if(LoRa_isvalid(_LoRa)){
		// goto sleep mode:
			LoRa_gotoMode(_LoRa, SLEEP_MODE);		//ok
			HAL_Delay(10);

		// turn on LoRa mode:
			read = LoRa_read(_LoRa, RegOpMode);		// ok
			HAL_Delay(10);
			data = read & 0x7F;
		 	LoRa_write(_LoRa, RegOpMode, data);
			HAL_Delay(100);
		...
		// set frequency:
			LoRa_setFrequency(_LoRa, _LoRa->frequency);		// ok

		// set output power gain:
			LoRa_setPower(_LoRa, _LoRa->power);		//ok

		// set over current protection:
			LoRa_setOCP(_LoRa, _LoRa->overCurrentProtection);		//ok

		// set LNA gain:
			LoRa_write(_LoRa, RegLna, 0x23);					// ok

		// set preamble:
			LoRa_write(_LoRa, FSK_RegPreambleMsb, _LoRa->preamble >> 8);
			LoRa_write(_LoRa, FSK_RegPreambleLsb, _LoRa->preamble >> 0);
