...
void delay100us( unsigned short iterations ) 
{
...
/*_________vvvvvvvvvvvvvvvvvvvvvv___________________________________________________________________________*/
...
//	MAP_GPIO_setOutputHighOnPin( pins->CSPort, pins->CSPin );
	...
	HAL_GPIO_WritePin(vFSK.CS_port, vFSK.CS_pin, GPIO_PIN_RESET);
	HAL_SPI_Transmit(vFSK.hSPIx, &addr, 1, TRANSMIT_TIMEOUT);
	while (HAL_SPI_GetState(vFSK.hSPIx) != HAL_SPI_STATE_READY);
	HAL_SPI_Transmit(vFSK.hSPIx, &data, 1, TRANSMIT_TIMEOUT);
	while (HAL_SPI_GetState(vFSK.hSPIx) != HAL_SPI_STATE_READY);
	HAL_GPIO_WritePin(vFSK.CS_port, vFSK.CS_pin, GPIO_PIN_SET);
}


/*_______vvvvvvvvvvvvvvvvvv_____________________________________________________________________________*/
uint8_t readRegister(unsigned char address){
//	// take the CS pin low to select the chip:
//    MAP_GPIO_setOutputLowOnPin( pins->CSPort, pins->CSPin );
...
//	return val;
	...
	HAL_GPIO_WritePin(vFSK.CS_port, vFSK.CS_pin, GPIO_PIN_RESET);
	HAL_SPI_Transmit(vFSK.hSPIx, &data_addr, 1, TRANSMIT_TIMEOUT);
	while (HAL_SPI_GetState(vFSK.hSPIx) != HAL_SPI_STATE_READY)
		;
	HAL_SPI_Receive(vFSK.hSPIx, &read_data, 1, RECEIVE_TIMEOUT);
	while (HAL_SPI_GetState(vFSK.hSPIx) != HAL_SPI_STATE_READY)
		;
	HAL_GPIO_WritePin(vFSK.CS_port, vFSK.CS_pin, GPIO_PIN_SET);
...
/*__________xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx__________________________________________________________________________*/
void readFSK(unsigned char address, unsigned char *data, unsigned char size) 
{
//	// take the CS pin low to select the chip:
//    MAP_GPIO_setOutputLowOnPin( pins->CSPort, pins->CSPin );
...
/*____________________________________________________________________________________*/
void writeFSK(unsigned char address, unsigned char *data, unsigned char size) 
{
//	// take the CS pin low to select the chip:
//    MAP_GPIO_setOutputLowOnPin( pins->CSPort, pins->CSPin );
...
/*____________________________________________________________________________________*/
unsigned char setOpMode(unsigned char opMode)
{
...
unsigned char GetFskBandwidthRegValue( unsigned long bandwidth)
{
	uint8_t i;
...
void setRxConfig( RxConfig_t* config )
{
...
void setTxConfig( TxConfig_t* config )
{
	setModem( config->modem );
...
/*____________________________________________________________________________________*/
void writeFifo(unsigned char *buffer, unsigned char size)
{
	writeFSK( 0, buffer, size );
}
/*____________________________________________________________________________________*/
void readFifo(unsigned char *buffer, unsigned char size)
{
	readFSK( 0, buffer, size );
}
/*____________________________________________________________________________________*/
unsigned char setIdleMode( bool idle )
{
	if (idle)
	{
...
/*____________________________________________________________________________________*/
unsigned char getRXData(unsigned char *data, unsigned char sz)
{
...
/*____________________________________________________________________________________*/
signed short GetRssi( RadioModems_t modem )
{
	signed short rssi = 0;
...
signed short getFrequencyError(){
	return (signed short)((double)(((unsigned short)readRegister(REG_FEIMSB) << 8 ) | ((unsigned short)readRegister(REG_FEILSB))) * FREQ_STEP);
}
